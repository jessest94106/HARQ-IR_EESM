...
%   Copyright 2021 The MathWorks, Inc.
    ...
    methods
        ...
        %   one HARQ entity per DL-SCH/UL-SCH transport channel per UE MAC entity.
            ...
            processstate.CumG = zeros(1,ncw);       % Cumulative coded bit capacity across HARQ attempts in current sequence
        ...
        %   See also updateAndAdvance.
            ...
            tbs = tbs + zeros(size(obj.NewData));               % Allow scalar expansion to per-codeword vectors
            g = g + zeros(size(obj.NewData));                   % Allow scalar expansion to per-codeword vectors
            if nargout
            ...
            cumg = obj.HarqProcessStates(stateidx).CumG;
            ...
            % Track cumulative bit capacity for effective code-rate reporting
            % over retransmissions of the same HARQ sequence.
            cumg(obj.NewData) = 0;
            cumg = cumg + g;
            cumg(~inerror | timeout) = 0;
            ...
            obj.HarqProcessStates(stateidx).CumG = cumg;
    ...
    methods (Access = private)
        ...
        function sr = createUpdateReport(harqEntity,blkerr,tbs,g)
            ...
            tbs = tbs + zeros(size(harqEntity.NewData));        % Allow scalar expansion to per-codeword vectors
            g = g + zeros(size(harqEntity.NewData));            % Allow scalar expansion to per-codeword vectors
            icr = tbs./g;    % Instantaneous code rate
            blkerr = logical(blkerr);
            stateidx = harqEntity.HarqProcessStateOrder(harqEntity.HarqProcessIndex);
            cumgPrev = harqEntity.HarqProcessStates(stateidx).CumG;
            cumgNow = cumgPrev + g;
            cumgNow(harqEntity.NewData) = g(harqEntity.NewData);
            ecr = tbs./cumgNow; % Effective code rate over attempts in this HARQ sequence
            ...
            for cw=1:length(harqEntity.NewData)
                ...
                % Parameters part
                ts2 = sprintf("(RV=%d,CR=%.0f/%.0f=%.6f,ECR=%.0f/%.0f=%.6f)", ...
                    harqEntity.RedundancyVersion(cw), ...
                    tbs(cw),g(cw),icr(cw), ...
                    tbs(cw),cumgNow(cw),ecr(cw));
